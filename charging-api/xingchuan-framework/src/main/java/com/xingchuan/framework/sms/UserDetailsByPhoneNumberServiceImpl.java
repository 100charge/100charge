...
import com.xingchuan.common.enums.UserStatus;
...
public class UserDetailsByPhoneNumberServiceImpl implements UserDetailsService {
    ...
    @Value("${user.register.defaultRoleId}")
    private Integer defaultRoleId;


    @Value("${user.register.enableTrialExpireTime}")
    private boolean enableTrialExpireTime;
    ...
    private SysUser addUser(String phoneNumber) {
        ...
        // 体验到期时间=当前时间+1天
        if (enableTrialExpireTime) {
            sysUser.setTrialExpireTime(DateUtils.addDays(new Date(), 2));
        }
        userService.insertUser(sysUser);

        Long userId = sysUser.getUserId();

        Long[] roleIds = new Long[]{defaultRoleId != null ? defaultRoleId.longValue() : null};
