...
import { pageStation, listMapStation, getOpenCities, getPendingPaymentOrder, getBannerList } from "@/config/api.js"
...
export default {
  ...
  watch: {
    showAuthLocation: {
      handler(val, old) {
        if (!val) {
          this.bindCityChange({ detail: { value: 0 } })
        }
      },
      immediate: false,
      deep: true,
  ...
  methods: {
          ...
          if (res.data.orderState == 1) {
            ...
            uni.showModal({
              ...
              success: (result) => {
                if (result.confirm) {
                  uni.navigateTo({
                    url: "/pages/stations/site/charging?orderNo=" + res.data.tradeNo,
                  })
                }
              },
            })
    ...
    getStationData() {
      let params = {
        ...
        chargingSpeed: this.stickySearchParams.speedValue || "", // 枚举类型,可用值:FAST_CHARGE,SLOW_CHARGE,SUPERCHARGE
        facilityLabel: this.stickySearchParams.installationLabel
          ? this.stickySearchParams.installationLabel.join(",")
          : "", // 设施
        parkingFeeLabel: this.stickySearchParams.parkLabel ? this.stickySearchParams.parkLabel.join(",") : "", // 停车费
        serviceLabel: this.stickySearchParams.serviceLabel ? this.stickySearchParams.serviceLabel.join(",") : "", // 服务
      }
        ...
        if (res && res.code == 200) {
          ...
          let rows = res.rows.map((item) => {
            return {
              ...
              distance: item.distance > 1000 ? (item.distance / 1000).toFixed(1) + " km" : item.distance + " m",
