...
/**
 * 获取某个日期往前推或往后推 n 天的日期
 * @param {string} date - 原始日期字符串（格式：YYYY-MM-DD）
 * @param {number} n - 天数（正数表示往后推，负数表示往前推）
 * @returns {string} - 返回新日期的字符串（格式：YYYY-MM-DD）
 */
export function getOffsetDate(date, n) {
...
// --> 7.17
export function formatDatePoint(val) {
