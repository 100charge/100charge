    ...
    <el-row :gutter="20">
      ...
      <el-col :span="4" :xs="24">
        <div class="left-container">
          <el-input v-model="deptName" placeholder="请输入组织架构名称" clearable prefix-icon="Search"
      ...
      <el-col :span="20" :xs="24">
        ...
        <el-table v-loading="loading" :data="userList" @selection-change="handleSelectionChange">
          ...
          <el-table-column label="创建时间" align="center" prop="createTime" v-if="columns[6].visible" width="100">
            <template #default="scope">
              <span>{{ parseTime(scope.row.createTime) }}</span>
            </template>
          </el-table-column>
          <el-table-column label="到期时间" align="center" prop="trialExpireTime" >
            <template #default="scope">
    ...
    <el-dialog :title="title" v-model="open" width="620px" append-to-body :close-on-click-modal="false">
      <el-form :model="form" :rules="rules" ref="userRef" label-width="80px">
          ...
          <el-col :span="12">
            <el-form-item label="所属部门" prop="deptId">
              <el-tree-select v-model="form.deptId" :data="deptOptions"
                :props="{ value: 'id', label: 'label', children: 'children' }" value-key="id" placeholder="请选择所属部门"
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="登录账号" prop="userName">
              <el-input v-model="form.userName" placeholder="请输入登录账号" maxlength="30" style="width: 260px"
                :disabled="form.userId">
              </el-input>
              <span style="font-size: 14px; color: #f56c6c; margin-left: 12px">提示：首次创建默认密码123456</span>
            </el-form-item>
          </el-col>
          <!-- <el-col :span="24">
        ...
        <el-row>
          <el-col :span="24" v-if="title == '修改子用户'">
...
function handleResetPwd(row) {
  proxy
    .$prompt('请输入"' + setUserName(row.userName) + '"的新密码', "提示", {
...
const handleFileSuccess = (response, file, fileList) => {
  ...
  proxy.$alert(
    ...
    (response.code == 200 ? response.msg : response.msg || "导入失败") +
    "</div>",
    "导入结果",
...
function handleFileError(e, file, files) {
  let err = JSON.parse(e.message)

  if (e.code !== 200) {
    proxy.$modal.msgError(err.msg || "导入失败，请重试！")
  }
}
/** 提交上传文件 */
...
function setUserName(value) {
  const suffix = "@" + useUserStore()?.tenant?.code
